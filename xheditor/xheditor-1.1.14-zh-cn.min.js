...
{ n: "Comic Sans MS" }, { n: "Courier New" }, { n: "System" }, { n: "Times New Roman" }, { n: "Tahoma" }, { n: "Verdana"}], T = [{ n: "x-small", s: "10px", t: "\u6781\u5c0f" }, { n: "small", s: "12px", t: "\u7279\u5c0f" }, { n: "medium", s: "16px", t: "\u5c0f" }, { n: "large", s: "18px", t: "\u4e2d" }, { n: "x-large", s: "24px", t: "\u5927" }, { n: "xx-large", s: "32px", t: "\u7279\u5927" }, { n: "-webkit-xxx-large", s: "48px", t: "\u6781\u5927"}], ib = [{ s: "\u5de6\u5bf9\u9f50", v: "justifyleft" }, { s: "\u5c45\u4e2d", v: "justifycenter" }, { s: "\u53f3\u5bf9\u9f50", v: "justifyright" }, { s: "\u4e24\u7aef\u5bf9\u9f50",
    ...
    h: 1
}, FontSize: { t: "\u5b57\u4f53\u5927\u5c0f", h: 1 }, Bold: { t: "\u52a0\u7c97 (Ctrl+B)", s: "Ctrl+B" }, Italic: { t: "\u659c\u4f53 (Ctrl+I)", s: "Ctrl+I" }, Underline: { t: "\u4e0b\u5212\u7ebf (Ctrl+U)", s: "Ctrl+U" }, Strikethrough: { t: "\u5220\u9664\u7ebf" }, FontColor: { t: "\u5b57\u4f53\u989c\u8272", h: 1 }, BackColor: { t: "\u80cc\u666f\u989c\u8272", h: 1 }, SelectAll: { t: "\u5168\u9009 (Ctrl+A)" }, Removeformat: { t: "\u5220\u9664\u6587\u5b57\u683c\u5f0f" }, Align: { t: "\u5bf9\u9f50", h: 1 }, List: { t: "\u5217\u8868", h: 1 }, Outdent: { t: "\u51cf\u5c11\u7f29\u8fdb" },
    Indent: { t: "\u589e\u52a0\u7f29\u8fdb" }, Link: { t: "\u8d85\u94fe\u63a5 (Ctrl+L)", s: "Ctrl+L", h: 1 }, Unlink: { t: "\u53d6\u6d88\u8d85\u94fe\u63a5" }, Anchor: { t: "\u951a\u70b9", h: 1 }, Img: { t: "\u56fe\u7247", h: 1 }, Flash: { t: "Flash\u52a8\u753b", h: 1 }, Media: { t: "\u591a\u5a92\u4f53\u6587\u4ef6", h: 1 }, Hr: { t: "\u63d2\u5165\u6c34\u5e73\u7ebf" }, Emot: { t: "\u8868\u60c5", s: "ctrl+e", h: 1 }, Table: { t: "\u8868\u683c", h: 1 }, Source: { t: "\u6e90\u4ee3\u7801" }, Preview: { t: "\u9884\u89c8" }, Print: { t: "\u6253\u5370 (Ctrl+P)", s: "Ctrl+P" }, Fullscreen: { t: "\u5168\u5c4f\u7f16\u8f91 (Esc)",
        s: "Esc"
    }, About: { t: "\u5173\u4e8e xhEditor" }, Word: { c: 'btnMap', t: '导入WORD文件', e: function () { var _this = this; _this.saveBookmark(); _this.showIframeModal('导入WORD文档', 'wordshow.aspx', function (v) { _this.loadBookmark(); _this.pasteHTML('' + v + ''); }, 700, 350); } }
...
Ia[e.trim(g.tools)]; g.tools = g.tools.match(/m?full/i) && xa ? $a.replace("Table", "Table," + ya) : $a
        ...
        g.tools.match(/(^|,)\s*About\s*(,|$)/i) || (g.tools += ",Fullscreen");
        g.tools = g.tools.split(",");

        if (g.editorRoot) A = g.editorRoot; !1 === eb && (A = V(A, "abs")); if (g.urlBase) g.urlBase = V(g.urlBase, "abs"); var ab = "xheCSS_" + g.skin, ja = "xhe" + aa + "_container", bb = "xhe" + aa + "_Tool", cb = "xhe" + aa + "_iframearea", db = "xhe" + aa + "_iframe", za = "xhe" + aa + "_fixffcursor", P = "", Q = "", ua = A + "xheditor_skin/" + g.skin + "/", Aa = kb, va = g.urlType, F = g.urlBase, Y = g.emotPath,
Y = Y ? Y : A + "xheditor_emot/", Ra = "", Aa = e.extend({}, Aa, g.emots), Y = V(Y, "rel", F ? F : null); (na = g.showBlocktag) && (Q += " showBlocktag"); var ma = []; this.init = function () {
...
a + '" _xhe_src="' + a + '"'); Z && (e += ' class="Apple-style-span"'); y && (e += ' class="' + y + '"'); return "<" + c + d + e + f
                }); h && (a = a.replace(/&apos;/ig, "&#39;")); if (!R) var c = function (a, b, c, d, e, f) {
                    var b = "", i, k; (i = d.match(/font-family\s*:\s*([^;"]+)/i)) && (b += ' face="' + i[1] + '"'); if (i = d.match(/font-size\s*:\s*([^;"]+)/i)) { i = i[1].toLowerCase(); for (var y = 0; y < T.length; y++) if (i === T[y].n || i === T[y].s) { k = y + 1; break } k && (b += ' size="' + k + '"', d = d.replace(/(^|;)(\s*font-size\s*:\s*[^;"]+;?)+/ig, "$1")) } if (k = d.match(/(?:^|[\s;])color\s*:\s*([^;"]+)/i)) {
                        if (i =
...
c), a = a.replace(/<(span)(\s+[^>]*?)?\s+style\s*=\s*"((?:[^"]*?;)?\s*(?:font-family|font-size|color)\s*:[^"]*)"( [^>]*)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?)<\/\1>/ig, c), a = a.replace(/<(span)(\s+[^>]*?)?\s+style\s*=\s*"((?:[^"]*?;)?\s*(?:font-family|font-size|color)\s*:[^"]*)"( [^>]*)?>(((?!<\1(\s+[^>]*?)?>)[\s\S])*?)<\/\1>/ig, c); a = a.replace(/<(td|th)(\s+[^>]*?)?>(\s|&nbsp;)*<\/\1>/ig, "<$1$2>" + (h ? "" : "<br />") + "</$1>")
            } else {
...
""); a = a.replace(/\r?\n/ig, ""); h ? (a = a.replace(/<v:shapetype(\s+[^>]*)?>[\s\S]*<\/v:shapetype>/ig, ""), a = a.replace(/<v:shape(\s+[^>]+)?>[\s\S]*?<v:imagedata(\s+[^>]+)?>\s*<\/v:imagedata>[\s\S]*?<\/v:shape>/ig, function (a, b, c) {
    if (a = c.match(/\s+src\s*=\s*("[^"]+"|'[^']+'|[^>\s]+)/i)) {
        a[1].match(/^(["']?)(.*)\1/); c = '<img src="' + A + 'xheditor_skin/blank.gif" _xhe_temp="true" class="wordImage"'; if (a = b.match(/\s+style\s*=\s*("[^"]+"|'[^']+'|[^>\s]+)/i)) a = a[1].match(/^(["']?)(.*)\1/)[2], c += ' style="' + a + '"'; return c +
" />"
    } return ""
})) : a = a.replace(/<img( [^<>]*(v:shapes|msohtmlclip)[^<>]*)\/?>/ig, function (a, b) { var c, d = '<img src="' + A + 'xheditor_skin/blank.gif" _xhe_temp="true" class="wordImage"'; (c = b.match(/ width\s*=\s*"([^"]+)"/i)) && (d += ' width="' + c[1] + '"'); (c = b.match(/ height\s*=\s*"([^"]+)"/i)) && (d += ' height="' + c[1] + '"'); return d + " />" }); for (var a = a.replace(/(<(\/?)([\w\-:]+))((?:\s+[\w\-:]+(?:\s*=\s*(?:"[^"]*"|'[^']*'|[^>\s]+))?)*)\s*(\/?>)/g, function (a, c, d, e, f, g) {
...
!0 === b) if (a.match(/^\s*$/)) H.push(a); else { var e = k[c]; e ? (d && v++, "" === A && v--) : A && v++; ((e ? c : "") !== A || e) && s(); H.push(a); "br" === c && s(); e && (D[c] || !d) && v--; A = e ? c : "" } else H.push(a)
                ...
                if (!c) return d; var e = "", c = c.replace(/ face\s*=\s*"\s*([^"]*)\s*"/i, function (a, b) { b && (e += "font-family:" + b + ";"); return "" }), c = c.replace(/ size\s*=\s*"\s*(\d+)\s*"/i, function (a, b) { e += "font-size:" + T[(7 < b ? 7 : 1 > b ? 1 : b) - 1].s + ";"; return "" }), c = c.replace(/ color\s*=\s*"\s*([^"]*)\s*"/i,
...
"" !== n ? b.attr("hspace", n) : b.removeAttr("hspace"), a[1] && (g = b.parent("a"), 0 === g.length && (b.wrap("<a></a>"), g = b.parent("a")), L(g, "href", a[1]), g.attr("target", "_blank"))))
            ...
            var b = e(b), p = d.getParent('embed[type="' + c + '"],embed[classid="' + f + '"]'), w = e("#xhe" + a + "Url", b), s = e("#xhe" + a + "Width", b), r = e("#xhe" + a + "Height", b), a = e("#xheSave", b); m && d.uploadInit(w, m, j); 1 === p.length && (w.val(L(p,
"src")), s.val(p.attr("width")), r.val(p.attr("height"))); a.click(function () {
    d.loadBookmark(); var a = w.val(); if ("" !== a && "http://" !== a) {
        var zmurl = w.val(); b = s.val(), j = r.val(), m = /^\d+%?$/; m.test(b) || (b = 412); m.test(j) || (j = 300);
        var oDate = new Date();
        var stime = oDate.getFullYear() + "" + (oDate.getMonth() + 1) + "" + oDate.getDate() + "" + oDate.getHours() + "" + oDate.getMinutes() + "" + oDate.getSeconds();
        var h = '<embed type="' + c + '" classid="' + f + '" src="xhe_tmpurl"' + g.replace("idzm_time", "idzm_" + stime).replace("xhe_tmpurl", zmurl).replace("xhe_tmpurl", zmurl).replace("xhe_tmpurl", zmurl).replace("xhe_width", b).replace("xhe_height", j), m = a.split(" "); if (1 < m.length) {
            var o, q = [], h = h + ' width="xhe_width" height="xhe_height" />', n; for (n in m) a = m[n].split("||"), o = h, o = o.replace("xhe_tmpurl", a[0]), o = o.replace("xhe_width", a[1] ? a[1] : b), o = o.replace("xhe_height", a[2] ? a[2] : j), "" !==
a && q.push(o);
...
X = null, d.focus(), d.loadBookmark())
            ...
            d.hidePanel(); var b = ka[a]; if (!b) return !1; if (null === v) { v = {}; var c = n.find(".xheButton[cmd=" + a + "]"); if (1 === c.length) v.target = c } if (b.e) b.e.call(d); else switch (a = a.toLowerCase(), a) {
...
"application/x-mplayer2", "clsid:6bf52a52-394a-11d3-b153-00c04f79faa6", ' enablecontextmenu="false" style="display:none" autostart="false" /><video id="idzm_time" class="video-js vjs-default-skin  vjs-big-play-centered" controls preload="none" width="xhe_width" height="xhe_height" poster="/html5play/play.jpg" data-setup="{}"><source src="xhe_tmpurl" type="video/mp4" /><source src="xhe_tmpurl" type="video/webm" /><source src="xhe_tmpurl" type="video/ogg" /></video><div style="display:none"', g.upMediaUrl, g.upMediaExt);
                    break; case "hr": d.pasteHTML("<hr color=red />"); break; case "emot": d.showEmot(); break; case "table": d.showTable(); break; case "source": d.toggleSource(); break; case "preview": d.showPreview(); break; case "print": W.print(); break; case "fullscreen": d.toggleFullscreen(); break; case "about": d.showAbout(); break; default: d._exec(a)
            } v = null
        }; this._exec = function (a, b, c) {
            c || d.focus(); return b !==
$ ? l.execCommand(a, !1, b) : l.execCommand(a, !1, null)
        } 
    }; ra.settings = { skin: "default", tools: "full", clickCancelDialog: !0, linkTag: !1, internalScript: !1, inlineScript: !1, internalStyle: !0, inlineStyle: !0, showBlocktag: !1, forcePtag: !0, upLinkExt: "zip,rar,txt", upImgExt: "jpg,jpeg,gif,png", upFlashExt: "swf", upMediaExt: "wmv,avi,wma,mp3,mid,mp4,flv", modalWidth: 350, modalHeight: 220, modalTitle: !0, defLinkText: "\u70b9\u51fb\u6253\u5f00\u94fe\u63a5", layerShadow: 3, emotMark: !1, upBtnText: "\u4e0a\u4f20", cleanPaste: 1, hoverExecDelay: 100, html5Upload: !0,
        upMultiple: 99
