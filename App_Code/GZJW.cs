...
public class GZJW
{
    ...
    public bool SyncClass()
    {
        ...
        DataTable dt = Sqlhelper.ConSerach(Sqlhelper.conStrzsgl, "select cl.bh PK_Class_NO,cl.xqdm FK_Campus_NO,spe.pk_spe FK_SPE_NO,zy.SPE_Code,cl.bjmc Name,cl.njdm Year from gzjw.[PantoSchoolGJ].[dbo].[JX_Class] cl left join enrollment.yxxt_data.dbo.Base_SPE_ZYDM zy on cl.zydm=zy.zydm left join enrollment.yxxt_data.dbo.Fresh_SPE spe on zy.spe_code=spe.spe_code left join enrollment.yxxt_data.dbo.Fresh_Counseller coun on cl.bh=coun.fk_class_no  where cl.njdm=@njdm and spe.[year]=@njdm", new SqlParameter("njdm", year));
        ...
        foreach (DataRow dr in dt.Rows)
        {
            ...
            x["Year"] = dr["Year"].ToString();
            temp.Rows.Add(x);
        }
        tempDs.Tables.Add(temp);
        Sqlhelper.GetUpdateDataSet("select PK_Class_NO,FK_Campus_NO,FK_SPE_NO,Name,[Year] from Fresh_Class", tempDs, "Fresh_Class");
