...
public partial class wblue_list : System.Web.UI.Page
{
        ...
        //显示菜单
            ...
            if (menus.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < menus.Rows.Count; i++)
                {
                    ...
                    //字菜单
                    DataTable zmenus = Sqlhelper.Serach("SELECT TOP 15 *  FROM [xw_lanm] where  fid='" + menus.Rows[i]["lmid"].ToString() + "' order by px");
            ...
            if(Request["id"]!=null)
            {
                ...
                if(lanms.Rows.Count>0)
                {
                   //读取新闻内容
                    ...
                    if (lanms.Rows[0]["lx"].ToString() == "图标")
                    {
                        //列表显示
                        //wp_content_w89_0.InnerHtml = "<div class=\"title\">列表显示！</div>";
                        wp_content_w89_0.Style.Add("display", "none");
                        wp_content_w89_1.Style.Add("display", "");
                        wp_content_w89_1.InnerHtml = "<div id=\"leamain\">       <iframe src=\"tb.aspx?id=" + Request["id"].ToString() + "\" marginheight=\"0\" marginwidth=\"0\" frameborder=\"0\" scrolling=\"no\" width=\"100%\" height=\"394\" id=\"iframepage\" name=\"iframepage\" onload=\"iFrameHeight()\"></iframe>";
                        wp_content_w89_1.InnerHtml+="<script type=\"text/javascript\" language=\"javascript\">function iFrameHeight() {";

        wp_content_w89_1.InnerHtml+="var ifm = document.getElementById(\"iframepage\");";

        wp_content_w89_1.InnerHtml+="var subWeb = document.frames ? document.frames[\"iframepage\"].document :ifm.contentDocument;";

       wp_content_w89_1.InnerHtml+=" if (ifm != null && subWeb != null) {          ifm.height = subWeb.body.scrollHeight;       }   }";

        wp_content_w89_1.InnerHtml+="function IsPC() {            var userAgentInfo = navigator.userAgent;            var Agents = new Array(\"Android\", \"iPhone\", \"SymbianOS\",\"Windows Phone\",\"mobile\",\"nokia\",\"samsung\",\"sonyericsson\",\"mot\",\"htc\",\"ucweb\");";
           wp_content_w89_1.InnerHtml+=" var flag = true;            for (var v = 0; v < Agents.length; v++) {                if (userAgentInfo.indexOf(Agents[v]) > 0) { flag = false; break; }";
           wp_content_w89_1.InnerHtml += "}            return flag;        }        if(!IsPC()) {window.location.href = \"tb.aspx?id=" + Request["id"].ToString() + "\";";
            wp_content_w89_1.InnerHtml += "}</script>      </div>";
              
        //DataTable xwnr = Sqlhelper.Serach("SELECT TOP 50 *  FROM [xw_neirong] where lmid=@xwid", new SqlParameter("xwid", Request["id"].ToString()));
        //if (xwnr.Rows.Count > 0)
        //{
        //    for (int i = 0; i < xwnr.Rows.Count; i++)
        //    {
        //        wp_content_w89_1.InnerHtml += " <li> <a href=\"xw.aspx?xwid=" + xwnr.Rows[i]["id"].ToString() + "\" target=\"_blank\" title=\"" + xwnr.Rows[i]["title"].ToString() + "\"><img src=\"" + xwnr.Rows[i]["xw_img"].ToString() + "\"></a><div class=\"cover\"><p></p><h3>" + xwnr.Rows[i]["title"].ToString() + "</h3>  </div>   </li>";
        //        //Response.Write(xwnr.Rows[i]["title"].ToString());
        ...
        //}
                    ...
                    if(lanms.Rows[0]["lx"].ToString()=="介绍")
                    {
                        ...
                        if(xws.Rows.Count>0)
                        {
                            if (xws.Rows[0]["wburl"].ToString().Trim().Length > 0)
                            {
                                Response.Redirect(xws.Rows[0]["wburl"].ToString().Trim());
                                Response.End();
                            }
                            string neirong=xws.Rows[0]["Content"].ToString();
                            ...
                            neirong = neirong.Replace("??", "").Replace("??", "").Replace("？", "").Replace("position:absolute;", "").Replace("??", "").Replace("??", "");
                            wp_content_w89_0.InnerHtml = "<p style=\"text-align:justify;text-indent:37px;font:12px/28px simsun;letter-spacing:normal;color:#4b4b4b;word-spacing:0px;font-size-adjust:none;font-stretch:normal;-webkit-text-stroke-width:0px\">" + neirong + "</p>";
                        }
                        else
                        {
                    ...
                    if(lanms.Rows[0]["fid"].ToString()=="-1")
                    {
                        ...
                        {
                            //跳转
                            if (lanms.Rows[0]["url"].ToString().Length > 4)
                            {
                                Response.Redirect(lanms.Rows[0]["url"].ToString());
                            }
                            else
                            {
                                //默认跳转到第一个子目录
                                urlok = "list.aspx?id=" + zlanms.Rows[0]["lmid"].ToString();
                                if (zlanms.Rows[0]["url"].ToString().Length > 4) urlok = zlanms.Rows[0]["url"].ToString();
