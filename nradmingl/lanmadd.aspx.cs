...
public partial class admin_lanmadd : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        {//添加栏目数据
            ...
            addStructLanm.font =this.TextBox2.Text.Trim();
            //若选择根目录，则fid字段设为－1
           ...
           //获取需要的权限验证
            ...
            string sqlInsertLanm = "INSERT INTO lanm(lmfont,lmmc,mcsm,fid,px,gjz,sfcdxs,sfdhxs,dhcdh,sfjlrz,sftop,sfqxyz,lmyyqx,lmtp,url,dkfs) VALUES(@lmfont,@lmmc,@mcsm,@fid,@px,@gjz,@sfcdxs,@sfdhxs,@dhcdh,@sfjlrz,@sftop,@sfqxyz,@lmyyqx,@lmtp,@url,@dkfs)";
            if (Sqlhelper.ExcuteNonQuery(sqlInsertLanm,
                new SqlParameter("lmfont", addStructLanm.font), new SqlParameter("lmmc", addStructLanm.lmmc),
        ...
        {//更改lanm表数据
            ...
            editStructLanm.font = this.TextBox2.Text.Trim();
            ...
            string sqlUpdateLanm = "UPDATE lanm SET lmfont=@lmfont,lmmc=@lmmc,mcsm=@mcsm,fid=@fid,px=@px,gjz=@gjz,sfcdxs=@sfcdxs,sfdhxs=@sfdhxs,dhcdh=@dhcdh,sfjlrz=@sfjlrz,sftop=@sftop,sfqxyz=@sfqxyz,lmyyqx=@lmyyqx,lmtp=@lmtp,url=@url,dkfs=@dkfs WHERE lmid=@lmid";
            if (Sqlhelper.ExcuteNonQuery(sqlUpdateLanm,
                new SqlParameter("lmfont", editStructLanm.font), new SqlParameter("lmmc", editStructLanm.lmmc),
    ...
    protected DataTable GetLanm()
    {
        try
        {
            string sqlStrGetFuLanm = "SELECT lmid,lmmc FROM lanm WHERE fid ='-1' or fid='0'";
            string sqlStruGetSecLanm = "Select a.lmid,a.lmmc from lanm a,(select lmid,lmmc,fid from lanm where fid='-1' or fid='0') b where a.fid = b.lmid";
    ...
    protected DataTable GetLanm(int lmid)
    {
        try
        {
            string sqlStrGetLanm = "SELECT * FROM lanm WHERE lmid=@lmid";
    ...
    protected DataTable GetLanm(string fid)
    {
        try
        {
            string sqlStrGetLanm = "SELECT * FROM lanm WHERE fid=@fid ";
    ...
    protected void editBind()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            this.Image2.ImageUrl = dt.Rows[0]["lmtp"].ToString();
            this.images2.InnerHtml = "<i class=\"" + dt.Rows[0]["lmfont"].ToString() + "\"></i>";
            editBindStructLanm.font= dt.Rows[0]["lmfont"].ToString();
        }
        else
        ...
        if (editBindStructLanm.fid != "-1" && editBindStructLanm.fid != "0")//为真，说明有子栏目
        {
            ...
            if (dtSerchDangQianlanm.Rows.Count > 0)
            {
                ...
                if (strDangQianlanmFid != "-1" && strDangQianlanmFid != "0")//为真，说明有三级栏目
                {
                    ...
                    DropDownList2.Items.FindByText(strDangQianLanmName).Selected = true;
        ...
        this.TextBox2.Text = editBindStructLanm.font;
        //填充栏目模块项
        rptAllEnablePower.DataSource = this.getQuanxian();
        rptAllEnablePower.DataBind();        
    ...
    {
        string sqlstring = "SELECT gjz FROM lanm WHERE gjz=@gjz";
        try
        {
    ...
    {
        this.images2.InnerHtml =  "<i class=\""+this.DropDownList3.SelectedValue+"\"></i>";
        this.TextBox2.Text = this.DropDownList3.SelectedValue;
    }
    protected void DropDownList4_SelectedIndexChanged(object sender, EventArgs e)
    {
        this.Image2.ImageUrl = this.DropDownList4.SelectedValue;
        this.TextBox1.Text = this.DropDownList4.SelectedValue;
    }
}