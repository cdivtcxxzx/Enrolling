...
public partial class admin_lanmadd_sub : System.Web.UI.Page
{
    //存修改栏目ID
    string fid = "";
    ...
    {        
        if (!IsPostBack)
        {
            if (Request["fid"] != null)
            {
                ...
                hrefFahui.PostBackUrl = "lanmgl_sub.aspx?fid=" + fid;
            }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        {//添加栏目数据
           ...
           //获取需要的权限验证
            ...
            string sqlInsertLanm = "INSERT INTO lanm(lmmc,mcsm,fid,px,gjz,sfcdxs,sfdhxs,dhcdh,sfjlrz,sftop,sfqxyz,lmyyqx,lmtp,url,dkfs) VALUES(@lmmc,@mcsm,@fid,@px,@gjz,@sfcdxs,@sfdhxs,@dhcdh,@sfjlrz,@sftop,@sfqxyz,@lmyyqx,@lmtp,@url,@dkfs)";
        ...
        {//更改lanm表数据
            ...
            string sqlUpdateLanm = "UPDATE lanm SET lmmc=@lmmc,mcsm=@mcsm,fid=@fid,px=@px,gjz=@gjz,sfcdxs=@sfcdxs,sfdhxs=@sfdhxs,dhcdh=@dhcdh,sfjlrz=@sfjlrz,sftop=@sftop,sfqxyz=@sfqxyz,lmyyqx=@lmyyqx,lmtp=@lmtp,url=@url,dkfs=@dkfs WHERE lmid=@lmid";
    ...
    /// 增加栏目页面初始化绑定数据
    /// </summary>
    protected void AddBind()
    {
        //获取根目录
        if (fid != "")
        {
            DataTable genglanmu1 = this.GetLanm(Int32.Parse(fid));
            ...
            if (genglanmu1 != null && genglanmu1.Rows.Count > 0)
            {
                this.dhcdhTiSshi.Text = "该栏目导航编号（菜单号）：" + genglanmu1.Rows[0]["dhcdh"].ToString();
                this.TB_Tree.Value = genglanmu1.Rows[0]["dhcdh"].ToString();
            ...
            DataTable lanmuxxByFid = this.GetLanm(fid);
            if (lanmuxxByFid != null && lanmuxxByFid.Rows.Count > 0)
            {
                DropDownList2.Visible = true;
                DropDownList2.DataSource = lanmuxxByFid;
                ...
                DropDownList2.DataValueField = "lmid";
                DropDownList2.DataBind();
                DropDownList2.Items.Insert(0, "选择子栏目");
            }
            else
            {
                DropDownList2.Items.Clear();
        ...
        if (editBindStructLanm.fid != "-1" && editBindStructLanm.fid != "0")//为真，说明有子栏目
        {
            ...
            if (dtSerchDangQianlanm.Rows.Count > 0)
            {
                ...
                if (strDangQianlanmFid != "-1" && strDangQianlanmFid != "0")//为真，说明有三级栏目
                {
                    ...
                    //DataTable dtFuLanmu = this.GetLanm(Int32.Parse(fid));
                    //dtFuLanmu.Merge(this.GetLanm("0"));
                    //DropDownList1.DataSource = dtFuLanmu;
                    //DropDownList1.DataTextField = "lmmc";
                    //DropDownList1.DataValueField = "lmid";
                    //DropDownList1.DataBind();
                    DropDownList1.Items.Insert(0, new ListItem(strYijiLanmName, fid));
                ...
                { //只有二级栏目
                    ...
                    //DropDownList1.DataValueField = "lmid";
                    //DropDownList1.DataBind();
                    DropDownList1.Items.Insert(0, new ListItem(strDangQianLanmName, fid));
