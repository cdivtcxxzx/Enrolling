...
public partial class admin_xwgl : System.Web.UI.Page
{
    ...
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "删除")
        {
            ...
            if (Sqlhelper.ExcuteNonQuery("DELETE FROM xibu_neirong  where id=" + id + " ") > 0)
            {
        ...
        try
        {
            ...
            for (int i = 0; i < chkIds.Length; i++)
            {
               ...
               // DataTable xm = Sqlhelper.Serach("select 姓名,领取状态 from byz where id=" + chkIds[i] + "");
                if (Sqlhelper.ExcuteNonQuery("DELETE FROM xibu_neirong  where id=" + chkIds[i] + " ") > 0)
                {
                   // this.Label1.Text = "<font color=green>新闻删除成功!</font>";
                    cg = cg + 1;
                }
    ...
    protected void Search_Onclick(object sender, ImageClickEventArgs e)
    {
        if (this.DropDownList2.SelectedValue.Length > 0)
        {
            ...
            this.SqlDataSource1.SelectCommand = "SELECT row_number() over (order by  xibu_neirong.fabutime desc)  AS 序号,xibu_lanm.lmmc, xibu_neirong.isyn,xibu_neirong.title, xibu_neirong.author, xibu_neirong.fabutime, xibu_neirong.images,xibu_neirong.id FROM xibu_neirong INNER JOIN xibu_lanm ON xibu_neirong.LMID = xibu_lanm.lmid where xibu_neirong.lmid='" + this.DropDownList2.SelectedValue + "' and xibu_neirong.title like '%" + this.searchtext.Text + "%'";
            ViewState["gridsql"] = SqlDataSource1.SelectCommand;
            SqlDataSource1.SelectCommand = ViewState["gridsql"].ToString();
            GridView1.DataBind();
        }
        else
        {
            ...
            this.SqlDataSource1.SelectCommand = "SELECT row_number() over (order by  xibu_neirong.fabutime desc)  AS 序号,xibu_lanm.lmmc,xibu_neirong.isyn, xibu_neirong.title, xibu_neirong.author, xibu_neirong.fabutime, xibu_neirong.images,xibu_neirong.id FROM xibu_neirong INNER JOIN xibu_lanm ON xibu_neirong.LMID = xibu_lanm.lmid where xibu_neirong.lmid='" + this.DropDownList1.SelectedValue + "' and xibu_neirong.title like '%" + this.searchtext.Text + "%'";
            ViewState["gridsql"] = SqlDataSource1.SelectCommand;
            SqlDataSource1.SelectCommand = ViewState["gridsql"].ToString();
            GridView1.DataBind();
        }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        if (this.DropDownList2.SelectedValue.Length > 0)
        {
            //有子栏目
            this.LB_top.Text = "新闻管理&gt;&gt;&nbsp;" + this.DropDownList1.SelectedItem.Text + "&gt;&gt;&nbsp;" + this.DropDownList2.SelectedItem.Text;
            this.SqlDataSource1.SelectCommand = "SELECT row_number() over (order by  xibu_neirong.fabutime desc)  AS 序号,xibu_lanm.lmmc,xibu_neirong.isyn, xibu_neirong.title, xibu_neirong.author, xibu_neirong.fabutime, xibu_neirong.images,xibu_neirong.id FROM xibu_neirong INNER JOIN xibu_lanm ON xibu_neirong.LMID = xibu_lanm.lmid where xibu_neirong.lmid='" + this.DropDownList2.SelectedValue + "'";
            ViewState["gridsql"] = SqlDataSource1.SelectCommand;
            SqlDataSource1.SelectCommand = ViewState["gridsql"].ToString();
            GridView1.DataBind();
        }
        else
        {
            this.LB_top.Text = "新闻管理&gt;&gt;&nbsp;" + this.DropDownList1.SelectedItem.Text;
            //无子栏目
            this.SqlDataSource1.SelectCommand = "SELECT row_number() over (order by  xibu_neirong.fabutime desc)  AS 序号,xibu_lanm.lmmc, xibu_neirong.title,xibu_neirong.isyn, xibu_neirong.author, xibu_neirong.fabutime, xibu_neirong.images,xibu_neirong.id FROM xibu_neirong INNER JOIN xibu_lanm ON xibu_neirong.LMID = xibu_lanm.lmid where xibu_neirong.lmid='" + this.DropDownList1.SelectedValue + "'";
            ViewState["gridsql"] = SqlDataSource1.SelectCommand;
            SqlDataSource1.SelectCommand = ViewState["gridsql"].ToString();
            GridView1.DataBind();
        }
