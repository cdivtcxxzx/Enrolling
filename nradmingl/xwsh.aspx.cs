...
public partial class admin_xwsh : System.Web.UI.Page
{
    #region 页面初始化参数

    private string pagelm1 = "新闻审核";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        this.SqlDataSource1.FilterExpression = new Power().Getlanm("glqx");
    }
    ...
    protected void Search_Onclick(object sender, ImageClickEventArgs e)
    {
        ...
        {
            //无子栏目
            this.LB_top.Text = "新闻审核&gt;&gt;&nbsp;" + this.DropDownList1.SelectedItem.Text;
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        if (this.DropDownList2.SelectedValue.Length > 0)
        {
            //有子栏目
            this.LB_top.Text = "新闻审核&gt;&gt;&nbsp;" + this.DropDownList1.SelectedItem.Text + "&gt;&gt;&nbsp;" + this.DropDownList2.SelectedItem.Text;
            this.SqlDataSource1.SelectCommand = "SELECT row_number() over (order by  xw_neirong.fabutime desc)  AS 序号,xw_lanm.lmmc,xw_neirong.isyn, xw_neirong.title, xw_neirong.author, xw_neirong.fabutime, xw_neirong.images,xw_neirong.id FROM xw_neirong INNER JOIN xw_lanm ON xw_neirong.LMID = xw_lanm.lmid where xw_neirong.lmid='" + this.DropDownList2.SelectedValue + "' and  xw_neirong.isyn='0' order by  xw_neirong.fabutime desc";
            ViewState["gridsql"] = SqlDataSource1.SelectCommand;
            SqlDataSource1.SelectCommand = ViewState["gridsql"].ToString();
            GridView1.DataBind();
        }
