...
public partial class nradmingl_bzr_xcbd : System.Web.UI.Page
{
    ...
    private string pagelm1 = "现场报到确认";//请与系统栏目管理中栏目关键字设置为一致便于权限管理
    ...
    protected string cscx(string images,string bdzt)
    {
        DataTable yfp = dormitory.serch_yfpbed(images);

        if (yfp.Rows.Count > 0)
        {
            return "<font color=blue>" + yfp.Rows[0][2].ToString() + "</font>";
        }
        if (bdzt == "已报到")
        {
            return "<a href=\"javascript: \" onclick=\"parent.layer.open({  type: 2,  title: '',  shadeClose: true,  shade: 0.8,  area: ['80%', '98%'],  content: '../view/ssfp-yfp.aspx?pk_affair_no=3&pk_sno=" + images + "&view=qxsh&pk_staff_no=" + images + "',cancel: function(index, layero){ location.reload(true)  }});\"    title=\"给未分寝室的学生分寝\">协助选寝</a>  ";
    ...
    protected void exportexcel(object sender, EventArgs e)
    {
        ...
        string filen = xzfile.DatatableToExcel(dt, this.DropDownList1.SelectedItem.Text+"学生详细数据");
    ...
    protected void gzt()
    {
        ...
        ViewState["gridsql"] = "SELECT TOP 200 row_number() over (order by  xm )  AS 序号,xh,[gkbmh] 高考报名号,[xm] 姓名,a.[gender_code] 性别,[sfzjh] 身份证号,[zc_zt] 网上注册,[ol_zt] 网上缴费,b.[Status_Code] 报到状态  FROM [TJ].[dbo].[Fresh_STU] a left join enrollment.yxxt_data.dbo.base_stu b on a.xh=b.pk_sno  where bjdm='" + this.DropDownList1.SelectedValue + "' " + sql2 + " order by xm";
    ...
    protected void Button1_Click1(object sender, EventArgs e)
    {
        ...
        ViewState["gridsql"] = "SELECT TOP 200 row_number() over (order by  xm )  AS 序号,xh,[gkbmh] 高考报名号,[xm] 姓名,[gender_code] 性别,[sfzjh] 身份证号,[zc_zt] 网上注册,[ol_zt] 网上缴费,[bd_zt] 报到状态  FROM [TJ].[dbo].[Fresh_STU] where bjdm='" + this.DropDownList1.SelectedValue + "' " + sql2 + " order by xm";
