...
public partial class admin_SchoolUpdate : System.Web.UI.Page
{
    string mode;
    string id;
    string yhid;
        ...
        if (!IsPostBack)
        {
            ...
            if (mode == "edit")
            {
                try
                {
                    ...
                    DataTable dt=Sqlhelper.Serach("select s.name,s.district_code code,y.yhid,y.guid from zy_school s left join yonghqx y on s.userguid=y.guid");
                    TB_xm.Text = dt.Rows[0]["name"].ToString();
                    TB_code.Text = id;
                    TB_code.Enabled = false;
                    TB_qx.Text = dt.Rows[0]["code"].ToString();
    ...
    protected void LB_insert_Click(object sender, EventArgs e)
    {
        ...
        string district_code = DropDownList3.SelectedValue;
        id = TB_code.Text;
        yhid = TB_yhid.Text;
        xm = TB_xm.Text;
        mm = TB_pwd.Text;
        string md5_mm = md5.MD5Encrypt(mm, md5.GetKey());
        string insertSchool = "insert into ZY_SCHOOL (school_id,name,district_code,userguid) values (@id,@name,@district_code,@userguid)";
        string insertYonghqx = "insert into yonghqx (guid,yhid,xm,mm) values (@guid,@yhid,@xm,@mm)";
        try
        {
            Sqlhelper.ExcuteNonQuery(insertSchool, new SqlParameter("id", id), new SqlParameter("name", xm), new SqlParameter("district_code", district_code), new SqlParameter("userguid", guid_new));
    ...
    protected void LB_up_Click(object sender, EventArgs e)
    {
        ...
        mm = TB_pwd.Text;
        string md5_mm = md5.MD5Encrypt(mm, md5.GetKey());
        string insertSchool = "update ZY_SCHOOL set name=@name,district_code=@code where school_id=@id";
        string insertYonghqx = "update yonghqx set xm=@xm,mm=@mm where guid=@guid";
        try
        {
            Sqlhelper.ExcuteNonQuery(insertSchool, new SqlParameter("id", id), new SqlParameter("name", xm), new SqlParameter("code", district_code));
    ...
    protected void DropDownList3_DataBound(object sender, EventArgs e)
    {
        ...
        DropDownList3.SelectedIndex = DropDownList3.Items.IndexOf(DropDownList3.Items.FindByText(TB_qx.Text));
    }
}