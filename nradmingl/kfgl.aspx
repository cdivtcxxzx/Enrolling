...
<asp:HiddenField ID="hdfWPBH" runat="server" />
    ...
    <asp:BoundField DataField="lmmc" HeaderText="栏目"  ControlStyle-CssClass="hidden-xs" HeaderStyle-CssClass="hidden-xs"  ItemStyle-CssClass="hidden-xs" SortExpression="lmmc"/>
   ...
   <asp:TemplateField HeaderText="状态"  ControlStyle-CssClass="hidden-xs" HeaderStyle-CssClass="hidden-xs"  ItemStyle-CssClass="hidden-xs"  SortExpression="title">
            ...
            <ItemTemplate>
            <a href="#" class="hidden-xs"><%# xwzt(Eval("isyn").ToString()) %></a>
            </ItemTemplate>
    ...
    <%--<asp:BoundField DataField="titleurl" HeaderText="标题"  HtmlEncode="false" HtmlEncodeFormatString="false" />--%>
        <asp:BoundField DataField="fabutime"  ControlStyle-CssClass="hidden-xs" HeaderStyle-CssClass="hidden-xs"  ItemStyle-CssClass="hidden-xs" HeaderText="发布时间" 
        ...
        <asp:TemplateField HeaderText="" >
                ...
                <ItemTemplate>
             <a href="javascript: " onclick="parent.layer.open({  type: 2,  title: '新闻编辑－<%# Eval("title").ToString() %>',  shadeClose: true,  shade: 0.8,  area: ['100%', '90%'],  content: 'xwfb.aspx?id=<%# Eval("id").ToString() %>'});"  txttop="txttop" class="layui-btn layui-btn-mini"  title="编辑新闻">编辑</a> &nbsp;&nbsp;<asp:LinkButton ID="LinkButton2" runat="server" CssClass="layui-btn layui-btn-danger layui-btn-mini" CommandName="删除"  CommandArgument='<%#Eval("id")%>'    OnClientClick="" CausesValidation="False"  Text='删除' >      
              </asp:LinkButton>
            </ItemTemplate>
