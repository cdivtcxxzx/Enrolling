...
public partial class admin_lanmadd_xw : System.Web.UI.Page
{
    ...
    protected void ListBox1_DataBound(object sender, EventArgs e)
    {
        if (Request["id"] != null)
        {
            DataTable xwneirong = Sqlhelper.Serach("select * from xw_lanm where lmid=" + Request["id"].ToString());
            int i = 0;
            // Response.Write(xwneirong.Rows[0]["readpower"].ToString());
            foreach (ListItem x in ListBox1.Items)
            {
                if (xwneirong.Rows[0]["glqx"].ToString().Contains(x.Value))
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        //public string glbm;
            //新添加项
            addStructLanm.topmenu = sftop0.Checked ? "1" : "0";
            addStructLanm.sfdown = sfdown.Checked ? "1" : "0";
            addStructLanm.sfsh = sfsh.Checked ? "1" : "0";
           // if (RBlist.Checked) addStructLanm.lx =RBlist.
        //addStructLanm.lx=this.RBClass.
        addStructLanm.oa = sfoafb.Checked ? "1" : "0";
        addStructLanm.logoimg = this.DropDownList4.SelectedValue.ToString().Trim();
        //管理权限
        string qx = "";
        foreach (ListItem x in ListBox1.Items)
        {
        ...
        addStructLanm.glqx = qx;
        addStructLanm.glbm = this.drglbm.SelectedValue.ToString().Trim();
        ...
        addStructLanm.lmmc = TB_Name0.Value.ToString().Trim();
           ...
           //获取需要的权限验证
            ...
            if (fileOK)
            {
                try
                {
                    addStructLanm.logoimg = DateTime.Now.ToShortTimeString();
                    FileUpload1.SaveAs(path + addStructLanm.logoimg);
                    addStructLanm.logoimg = "images/" + addStructLanm.logoimg;
            ...
            //END ADD 展示类型 by chenzhiqiu 20150114        //public string sfdown;
            //public string oa;
            //public string logoimg;
            //public string glqx;
            //public string glbm;
            string sqlInsertLanm = "INSERT INTO xw_lanm(sfsh,sfdown,glqx,logoimg,glbm,oa,lmmc,mcsm,lx,fid,px,gjz,sfcdxs,sfdhxs,dhcdh,sfjlrz,sftop,sfqxyz,lmyyqx,lmtp,url,topmenu,dkfs) VALUES(@sfsh,@sfdown,@glqx,@logoimg,@glbm,@oa,@lmmc,@mcsm,@lx,@fid,@px,@gjz,@sfcdxs,@sfdhxs,@dhcdh,@sfjlrz,@sftop,@sfqxyz,@lmyyqx,@lmtp,@url,@topmenu,@dkfs)";
            if (Sqlhelper.ExcuteNonQuery(sqlInsertLanm,
                new SqlParameter("sfsh",addStructLanm.sfsh),
                new SqlParameter("sfdown", addStructLanm.sfdown),
                new SqlParameter("glqx", addStructLanm.glqx),
                new SqlParameter("logoimg", addStructLanm.logoimg),
                new SqlParameter("glbm", addStructLanm.glbm),
                new SqlParameter("oa", addStructLanm.oa),
                ...
                new SqlParameter("topmenu", addStructLanm.topmenu),
        ...
        {//更改lanm表数据
            ...
            //public string glbm;
            //新添加项
            editStructLanm.topmenu = sftop0.Checked ? "1" : "0";
            editStructLanm.sfsh = sfsh.Checked ? "1" : "0";
            editStructLanm.sfdown = sfdown.Checked ? "1" : "0";
            // if (RBlist.Checked) addStructLanm.lx =RBlist.
            //addStructLanm.lx=this.RBClass.
            editStructLanm.oa = sfoafb.Checked ? "1" : "0";
            editStructLanm.logoimg = this.DropDownList4.SelectedValue.ToString().Trim();
            //管理权限
            string qx = "";
            foreach (ListItem x in ListBox1.Items)
            {
            ...
            editStructLanm.glqx = qx;
            editStructLanm.glbm = this.drglbm.SelectedValue.ToString().Trim();
            ...
            if (fileOK)
            {
                ...
                {
                    editStructLanm.logoimg = DateTime.Now.ToShortTimeString();
                    FileUpload1.SaveAs(path +editStructLanm.logoimg);
                    editStructLanm.logoimg = "images/" + editStructLanm.logoimg;
            ...
            string sqlUpdateLanm = "UPDATE xw_lanm SET sfsh=@sfsh,sfdown=@sfdown,glqx=@glqx,logoimg=@logoimg,glbm=@glbm,oa=@oa,lmmc=@lmmc,mcsm=@mcsm,lx=@lx,fid=@fid,px=@px,gjz=@gjz,sfcdxs=@sfcdxs,sfdhxs=@sfdhxs,dhcdh=@dhcdh,sfjlrz=@sfjlrz,sftop=@sftop,sfqxyz=@sfqxyz,lmyyqx=@lmyyqx,lmtp=@lmtp,topmenu=@topmenu,url=@url,dkfs=@dkfs WHERE lmid=@lmid";
            if (Sqlhelper.ExcuteNonQuery(sqlUpdateLanm,
                new SqlParameter("sfsh",editStructLanm.sfsh),
                new SqlParameter("sfdown", editStructLanm.sfdown),
                new SqlParameter("glqx", editStructLanm.glqx),
                new SqlParameter("logoimg", editStructLanm.logoimg),
                new SqlParameter("glbm", editStructLanm.glbm),
                new SqlParameter("oa", editStructLanm.oa),
                ...
                new SqlParameter("topmenu", editStructLanm.topmenu),
    ...
    protected void editBind()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            editBindStructLanm.topmenu = dt.Rows[0]["topmenu"].ToString();
            ...
            editBindStructLanm.sfdown = dt.Rows[0]["sfdown"].ToString();
            editBindStructLanm.oa = dt.Rows[0]["oa"].ToString();
            editBindStructLanm.logoimg = dt.Rows[0]["logoimg"].ToString();
            editBindStructLanm.glqx = dt.Rows[0]["glqx"].ToString();
            editBindStructLanm.glbm = dt.Rows[0]["glbm"].ToString();
            editBindStructLanm.sfsh = dt.Rows[0]["sfsh"].ToString();
        ...
        if (editBindStructLanm.topmenu == "1") sftop0.Checked = true; else sftop0.Checked = false;
        if (editBindStructLanm.sfdown == "1") sfdown.Checked = true; else sfdown.Checked = false;
        if (editBindStructLanm.oa == "1") sfoafb.Checked = true; else sfoafb.Checked = false;
        if (editBindStructLanm.sfsh == "1") sfsh.Checked = true; else sfsh.Checked = false;
        //public string sfdown;
        //public string oa;
        ...
        try
        {
            DropDownList3.Items.FindByValue(editBindStructLanm.lmtp).Selected = true;
            Image1.ImageUrl = editBindStructLanm.lmtp;
        ...
        try
        {
            DropDownList4.Items.FindByValue(editBindStructLanm.logoimg).Selected = true;
            Image2.ImageUrl = editBindStructLanm.logoimg;
        }
        catch
        {
        ...
        //Response.Write(editBindStructLanm.glbm);
        try
        {
            drglbm.DataBind();
            drglbm.Items.FindByValue(editBindStructLanm.glbm).Selected = true;
        }
        catch
        {
        }
    ...
    {
        this.Image2.ImageUrl = this.DropDownList4.SelectedValue;
        this.images1.Text = this.DropDownList4.SelectedValue;
    }
