<%@ Page Language="C#" AutoEventWireup="true" CodeFile="xwglmy.aspx.cs" Inherits="admin_xwglmy" %>
...
<body>
	...
	<tr>
		<td>
        <asp:LinkButton runat="server" ID="LB_top" Text="信息管理&gt;&gt;&nbsp;我的全部新闻信息"></asp:LinkButton></td><td style="text-align:right">
...
&nbsp;</td>
    ...
    <asp:BoundField DataField="lmmc" HeaderText="栏目" SortExpression="lmmc"/>
     <asp:TemplateField HeaderText="标题"  SortExpression="title">
            ...
            <ItemTemplate>
            <a href="/xw.aspx?xwid=<%# Eval("id").ToString() %>"  txttop="txttop"  target="_blank"  title="点击查看新闻信息"><%# Eval("title").ToString() %><%# imagestu(Eval("images").ToString()) %></a>
            </ItemTemplate>
   ...
   <asp:TemplateField HeaderText="状态"  SortExpression="title">
      ...
      <asp:BoundField DataField="xm" HeaderText="发布人" SortExpression="xm" />
     <asp:BoundField DataField="shenpr" HeaderText="审批" SortExpression="shenpr" />
...
<span style="float:left;">
        ...
        <asp:SqlDataSource onselected="SqlDataSource1_Selected"   ID="SqlDataSource1" runat="server" 
            ...
            SelectCommand="SELECT   row_number() over (order by xw_neirong.fabutime desc) AS 序号,  xw_lanm.lmmc, xw_neirong.title, yonghqx.xm,xw_neirong.shenpr, xw_neirong.fabutime, xw_neirong.images, xw_neirong.isyn, xw_neirong.id, yonghqx.xm
FROM         xw_neirong LEFT OUTER JOIN
                      xw_lanm ON xw_neirong.LMID = xw_lanm.lmid LEFT OUTER JOIN
                      yonghqx ON xw_neirong.fabuyhid = yonghqx.yhid
WHERE     (xw_neirong.fabuyhid = @fabuyhid) OR
                      (xw_neirong.shenpr =@fabuyhid)
ORDER BY xw_neirong.fabutime DESC">
            <SelectParameters>
                <asp:SessionParameter Name="fabuyhid" SessionField="UserName" Type="String" />
            </SelectParameters>
        </asp:SqlDataSource>
    </div>
    <asp:SqlDataSource ID="SqlDataSource3" runat="server" 
        ...
        SelectCommand="SELECT [lmid], [lmmc], [px],[glqx]  FROM [xw_lanm] WHERE ([fid] = @fid) ORDER BY [px]">
                    <SelectParameters>
